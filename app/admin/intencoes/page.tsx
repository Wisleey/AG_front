...
export default function AdminIntencoesPage() {
  ...
  const [selectedIntencao, setSelectedIntencao] = useState<Intencao | null>(
    null
  );
  ...
  const confirmarAcao = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || err.message || "Erro ao processar ação"
      );
    }
  };
            ...
            {intencoes.length === 0 ? (
              ...
              >
                <CardBody className="text-center py-16">
                  <svg
                    className="w-16 h-16 text-neutral-500 mx-auto mb-4"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
